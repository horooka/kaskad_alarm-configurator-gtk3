...
const unsigned char ALARMOBJTYPE_NONE = 0;
const unsigned char ALARMOBJTYPE_KLOGICPARAM = 1;
const unsigned char ALARMOBJTYPE_GROUPALARM = 2;
const unsigned char ALARMOBJTYPE_SERVICEPARAMS = 4;
const unsigned char ALARMOBJTYPE_KLOGICVARPARAM = 8;
const unsigned char ALARMOBJTYPE_VARPARAMS = 16;

const unsigned char KLOGICTYPE_FLOAT = 0;
const unsigned char KLOGICTYPE_BOOL = 1;
const unsigned char KLOGICTYPE_INT = 2;
const unsigned char PASSPORTTYPE_ALARM = 111;
const unsigned char PASSPORTTYPE_USERDEF = 222;
const unsigned char PASSPORTTYPE_SERVICE = 223;
...
class AlarmObjCols : public Gtk::TreeModel::ColumnRecord {
    public:
        ...
        Gtk::TreeModelColumn<int> passport_id;
...
class SituationCols : public Gtk::TreeModel::ColumnRecord {
    public:
        SituationCols() {
            ...
            add(params_vals);
            add(sounds);
            add(launch_program_file);
            add(launch_program_params);
            add(checkboxes);
            add(params);
            add(insense_absolute);
            add(insense_up);
            add(insense_down);
        }
        ...
        Gtk::TreeModelColumn<Glib::ustring> launch_program_file;
        Gtk::TreeModelColumn<Glib::ustring> launch_program_params;
        // Защелкивать ситуацию, писать события, гистерезис, SMS, e-mail, 
        // отслеживать ситуацию по паспорту, запись в паспорт при срабатывании,
        // запускать программу
        Gtk::TreeModelColumn<std::array<bool, 8>> checkboxes;
        Gtk::TreeModelColumn<std::array<SituationParams, 4>> params;
        // Гистерезис
        Gtk::TreeModelColumn<bool> insense_absolute;
        Gtk::TreeModelColumn<double> insense_up;
        Gtk::TreeModelColumn<double> insense_down;
};
...
bool parse_alarms_configs(const std::string &alarms_dir_path, std::vector<UsedAlarmRef> &used_params, Glib::RefPtr<Gtk::TreeStore> &treestore_stations, const std::array<Glib::RefPtr<Gdk::Pixbuf>, 5> &pixbufs_loop, const GroupCols &groups_cols, const AlarmObjCols &alarmobj_cols, const SituationCols &situation_cols, const SoundCols &sound_cols, const ContactCols &contact_cols, std::string &errors);
bool parse_alarms_config(const std::string &alarms_dir_path, const Gtk::TreeModel::Row &station_row, std::vector<UsedAlarmRef> &used_params, Glib::RefPtr<Gtk::TreeStore> &treestore_stations, const std::array<Glib::RefPtr<Gdk::Pixbuf>, 5> &pixbufs_loop, const GroupCols &groups_cols, const AlarmObjCols &alarmobj_cols, const SituationCols &situation_cols, const SoundCols &sound_cols, const ContactCols &contact_cols, std::string &errors);
...
bool parse_controller_bin(int cntrl_id, const std::string &controller_config_path, const Gtk::TreeModel::Children &zones_station_children, const Gtk::TreeModel::Row &station_row, Glib::RefPtr<Gtk::TreeStore> &treestore_stations, const std::vector<UsedAlarmRef> &used_params,
                          ...
                          const Glib::RefPtr<Gdk::Pixbuf> &pixbuf_discrete, const Glib::RefPtr<Gdk::Pixbuf> &pixbuf_analog, const Glib::RefPtr<Gdk::Pixbuf> &pixbuf_variables_group,
                          const Glib::RefPtr<Gdk::Pixbuf> &pixbuf_variables_array, const std::array<Glib::RefPtr<Gdk::Pixbuf>, 6> &pixbufs_params,
                          const AlarmObjCols &alarmobj_cols, const GroupCols &group_cols, std::string &errors);
bool parse_klogic_bins(const std::string &alarms_dir_path, Glib::RefPtr<Gtk::TreeStore> &treestore_stations, const std::vector<UsedAlarmRef> &used_params,

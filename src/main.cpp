...
class AlarmConfigurator : public Gtk::Window {
    ...
    private:
        ...
        void swap_situation_rows(Gtk::TreeModel::Row &row1,
            ...
            std::array<bool, 8> temp_array_bool;
            ...
            if (selected_passports_paths.empty()) {
                ...
                for (auto it = selected_passports_paths.rbegin();
                    ...
                    Gtk::TreeModel::Row klogicparam_row =
                        get_klogicparam_row_by_alarm_row(*iter, treestore_zones,
                                                         alarmobj_cols);
                    if (!klogicparam_row)
                        continue;
        ...
        Passport on_passport_choice(unsigned char value_type) {
            Gtk::MessageDialog dialog(std::string("Выбор параметра") +
                                          (value_type == 1   ? " (Дискретный)"
                                           : value_type == 2 ? " (Аналоговый)"
                                                             : ""),
            ...
            treeview_tree->get_selection()->signal_changed().connect(
                [this, treeview_tree, filter_tree, value_type]() {
                    ...
                    for (const Gtk::TreeModel::Row &param_row_src :
                        ...
                        if ((param_row_src[alarmobj_cols.alarmobj_type] &
                            ...
                            continue;
                        if (value_type == 1 &&
                            param_row_src[alarmobj_cols.type] !=
                                KLOGICTYPE_BOOL)
                            continue;
            ...
            button_passport_choice->signal_clicked().connect(
                [this, radiobutton_setpoint_passport, curr_group_row,
                 ...
                 label_passport_name_val, setpoint_idx]() {
                    Passport new_passport = on_passport_choice(0);
        ...
        fill_alarm_choice_recursive(const Gtk::TreeModel::Row &row_src,
            ...
            auto append_alarm_passport = [this, &treestore_dst, children_dst,
                ...
                row[alarmobj_cols.passport_type] = PASSPORTTYPE_ALARM;
        ...
        void redraw_alarms_params() {
            ...
            entry_filename.set_text(
                curr_situation_row[situation_cols.launch_program_file]);
            entry_cmd_parameters.set_text(
                curr_situation_row[situation_cols.launch_program_params]);
            std::array<bool, 8> checkboxes =
                curr_situation_row[situation_cols.checkboxes];
            ...
            checkbutton_launch_program.set_active(checkboxes[7]);
            vbox_launch_program.set_sensitive(checkboxes[7]);
        ...
        void redraw_alarms_hysteresis() {
            ...
            if (!situation_additional_menus[0]) {
                ...
                spin_higher_hysteresis.signal_value_changed().connect([this]() {
                    ...
                    double old = curr_situation_row[situation_cols.insense_up];
                    if (old == val ||
                        (std::isfinite(old) && std::isfinite(val) &&
                         std::abs(old - val) <= 1e-18 * (1.0 + std::abs(old))))
                        return;
                    curr_situation_row[situation_cols.insense_up] = val;
                    button_save_enable();
                });
                ...
                spin_lower_hysteresis.signal_value_changed().connect([this]() {
                    ...
                    double old =
                        curr_situation_row[situation_cols.insense_down];
                    if (old == val ||
                        (std::isfinite(old) && std::isfinite(val) &&
                         std::abs(old - val) <= 1e-18 * (1.0 + std::abs(old))))
                        return;
                    curr_situation_row[situation_cols.insense_down] = val;
                    button_save_enable();
                });
                ...
                combobox_hysteresis.signal_changed().connect([this]() {
                    ...
                    if (curr_situation_row[situation_cols.insense_absolute] ==
                        val)
                        return;
                    curr_situation_row[situation_cols.insense_absolute] = val;
                    button_save_enable();
                });
            ...
            spin_higher_hysteresis.set_value(
                curr_situation_row[situation_cols.insense_up]);
            spin_lower_hysteresis.set_value(
                curr_situation_row[situation_cols.insense_down]);
            combobox_hysteresis.set_active(
                curr_situation_row[situation_cols.insense_absolute] ? 1 : 0);
        ...
        void setup_gresources() {
            ...
            pixbuf_variables_group = Gdk::Pixbuf::create_from_resource(
                "/org/icons/variables-group.png");
            pixbuf_variables_array = Gdk::Pixbuf::create_from_resource(
                "/org/icons/variables-array.png");
        }
        ...
        void setup_menus() {
            ...
            frame_launch_program.set_label_widget(checkbutton_launch_program);
            vbox_alarm_parameters.pack_start(frame_launch_program,
                                             Gtk::PACK_SHRINK);
            frame_launch_program.add(vbox_launch_program);
            checkbutton_launch_program.signal_toggled().connect([this]() {
                Gtk::TreeModel::Row curr_situation_row =
                    *treeview_situations.get_selection()->get_selected();
                if (!curr_situation_row)
                    return;
                std::array<bool, 8> checkboxes =
                    curr_situation_row[situation_cols.checkboxes];
                if (checkboxes[7] == checkbutton_launch_program.get_active())
                    return;
                checkboxes[7] = checkbutton_launch_program.get_active();
                curr_situation_row[situation_cols.checkboxes] = checkboxes;
                vbox_launch_program.set_sensitive(checkboxes[7]);
                button_save_enable();
            });
            vbox_launch_program.pack_start(hbox_launch_program_file,
                                           Gtk::PACK_SHRINK);
            hbox_launch_program_file.pack_start(label_filename,
                                                Gtk::PACK_SHRINK);
            hbox_launch_program_file.pack_start(entry_filename,
                                                Gtk::PACK_EXPAND_WIDGET);
            entry_filename.signal_changed().connect([this]() {
                Gtk::TreeModel::Row curr_situation_row =
                    *treeview_situations.get_selection()->get_selected();
                if (!curr_situation_row)
                    return;
                if (curr_situation_row[situation_cols.launch_program_file] ==
            ...
            hbox_launch_program_file.pack_end(button_select_program_file,
                                              Gtk::PACK_SHRINK);
            button_select_program_file.signal_clicked().connect([this]() {
                Gtk::FileChooserDialog dialog("Выберите программу",
                                              Gtk::FILE_CHOOSER_ACTION_OPEN);
                dialog.add_button("Отмена", Gtk::RESPONSE_CANCEL);
                dialog.add_button("OK", Gtk::RESPONSE_OK);
                dialog.set_filename(entry_filename.get_text());
                if (dialog.run() == Gtk::RESPONSE_OK) {
                    std::string filename = dialog.get_filename();
            ...
            vbox_launch_program.pack_start(hbox_launch_program_parameters,
                                           Gtk::PACK_SHRINK);
            hbox_launch_program_parameters.pack_start(label_cmd_parameters,
                                                      Gtk::PACK_SHRINK);
            hbox_launch_program_parameters.pack_start(entry_cmd_parameters,
                                                      Gtk::PACK_EXPAND_WIDGET);
            entry_cmd_parameters.signal_changed().connect([this]() {
                Gtk::TreeModel::Row curr_situation_row =
                    *treeview_situations.get_selection()->get_selected();
                if (!curr_situation_row)
                    return;
                if (curr_situation_row[situation_cols.launch_program_params] ==
                    entry_cmd_parameters.get_text())
                    return;
                curr_situation_row[situation_cols.launch_program_params] =
                    entry_cmd_parameters.get_text();
                button_save_enable();
            });
            set_margin(vbox_launch_program, 10, 5);
        ...
        void setup_signals() {
            ...
            button_situation_new_expand.signal_clicked().connect([this]() {
                ...
                auto new_situation_button = [this, popover, vbox,
                    ...
                    button_alarm->signal_clicked().connect([this, popover,
                        ...
                        new_row[situation_cols.insense_absolute] = false;
                        new_row[situation_cols.insense_up] = 0.0;
                        new_row[situation_cols.insense_down] = 0.0;
                    ...
                    if (dst_station_id != curr_station_id) {
                        if (unsaved) {
                            ...
                            if (res == Gtk::RESPONSE_YES) {
                                ...
                                Gtk::TreeModel::Children
                                    new_zones_station_children =
                                        new_station_row.children();
                                // Восстановление станции в дереве KLogic при
                                // отмене изменений
                                fastparam::Machins *m =
                                ...
                                for (int i = 0; i < n; i++) {
                                    ...
                                    (void)parse_controller_bin(
                                        ...
                                        pixbuf_analog, pixbuf_variables_group,
                                        pixbuf_variables_array, pixbufs_params,
            ...
            treeview_passports.signal_drag_data_received().connect(
                [this](const Glib::RefPtr<Gdk::DragContext> &context, int x,
                    ...
                    unsigned char source_alarmobj_type =
                        source_row[alarmobj_cols.alarmobj_type];
                    new_row[alarmobj_cols.alarmobj_type] =
                        ALARMOBJTYPE_GROUPALARM |
                        (source_alarmobj_type & ALARMOBJTYPE_KLOGICVARPARAM);
                    ...
                    new_row[alarmobj_cols.passport_priority] =
                        is_station() ? 3 : 2;
                    new_row[alarmobj_cols.station_id] =
                        static_cast<short unsigned int>(
                            source_row[alarmobj_cols.station_id]);
                    ...
                    new_row[alarmobj_cols.passport_group_id] =
                        static_cast<short unsigned int>(
                            source_row[alarmobj_cols.passport_group_id]);
                    new_row[alarmobj_cols.passport_type] =
                        static_cast<unsigned char>(
                            source_row[alarmobj_cols.passport_type]);
                    ...
                    new_row[alarmobj_cols.klogic_string_id] =
                        static_cast<Glib::ustring>(
                            source_row[alarmobj_cols.klogic_string_id]);
                    new_row[alarmobj_cols.alarm_string_id] =
                        static_cast<Glib::ustring>(
                            source_row[alarmobj_cols.alarm_string_id]);
                    source_row[alarmobj_cols.userpath] =
        ...
        void setup_data() {
            ...
            (void)parse_klogic_bins(
                ...
                pixbuf_variables_group, pixbuf_variables_array, pixbufs_params,
                alarmobj_cols, group_cols, errors);
                ...
                if (unsaved) {
                    ...
                    if (res == Gtk::RESPONSE_YES) {
                        ...
                        Gtk::TreeModel::Children zones_station_children =
                            treestore_zones->children().begin()->children();
                        Gtk::TreeModel::Row new_station_row;
                        for (const Gtk::TreeModel::Row &row :
                             zones_station_children) {
                            if (row[alarmobj_cols.chipher] ==
                        ...
                        Gtk::TreeModel::Children new_zones_station_children =
                            new_station_row.children();
                        // Восстановление станции в дереве KLogic при отмене
                        // изменений
                        ...
                        for (int i = 0; i < n; i++) {
                            ...
                            (void)parse_controller_bin(
                                ...
                                new_zones_station_children, curr_station_row,
            ...
            treeview_zones.expand_row(Gtk::TreeModel::Path("0"), false);
            treeview_station.get_selection()->select(
                filter_groups->children().begin());
        }
        ...
        Glib::RefPtr<Gdk::Pixbuf> pixbuf_save_enabled, pixbuf_save_disabled,
            ...
            pixbuf_variables_group, pixbuf_variables_array;
        ...
        Gtk::CheckButton checkbutton_launch_program{"Запускать программу"};
        Gtk::Frame frame_launch_program;
        Gtk::Box vbox_launch_program{Gtk::ORIENTATION_VERTICAL, 5};
        Gtk::Box hbox_launch_program_file{Gtk::ORIENTATION_HORIZONTAL, 5};
        Gtk::Label label_filename{"Имя файла"};
        Gtk::Entry entry_filename;
        Gtk::Button button_select_program_file{"Выбрать"};
        Gtk::Box hbox_launch_program_parameters{Gtk::ORIENTATION_HORIZONTAL, 5};
        Gtk::Label label_cmd_parameters{"Параметры командной строки"};

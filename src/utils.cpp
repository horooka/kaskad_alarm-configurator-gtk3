...
Gtk::TreeModel::Row get_klogicparam_row_by_string_id(
    ...
    std::vector<Glib::ustring> string_id_parts;
    std::size_t start = 0;
    std::size_t end;
    while ((end = string_id.find('.', start)) != std::string::npos) {
        string_id_parts.push_back(
            Glib::ustring(string_id.substr(start, end - start)));
        start = end + 1;
    }
    string_id_parts.push_back(
        Glib::ustring(string_id.substr(start, string_id.size())));
    Gtk::TreeModel::Row klogic_row = *treestore_zones->children().begin();
    for (const Gtk::TreeModel::Row &station_row : klogic_row.children()) {
        if (string_id_parts[0] != station_row[alarmobj_cols.chipher])
            continue;
        for (const Gtk::TreeModel::Row &controller_row :
             station_row.children()) {
            if (string_id_parts[1] != controller_row[alarmobj_cols.name])
                continue;
            for (const Gtk::TreeModel::Row &task_row :
                 controller_row.children()) {
                if (task_row[alarmobj_cols.alarmobj_type] ==
                    ALARMOBJTYPE_SERVICEPARAMS) {
                    for (const Gtk::TreeModel::Row &param_row :
                         task_row.children()) {
                        if (string_id_parts[3] ==
                            param_row[alarmobj_cols.name]) {
                            return param_row;
                        }
                    }
                }
                if (string_id_parts[2] != task_row[alarmobj_cols.name])
                    continue;
                for (const Gtk::TreeModel::Row &fb_row : task_row.children()) {
                    if (string_id_parts[3] != fb_row[alarmobj_cols.name])
                        continue;
                    for (const Gtk::TreeModel::Row &param_row :
                         fb_row.children()) {
                        if (string_id_parts[4] ==
                            param_row[alarmobj_cols.name]) {
                            return param_row;
                        }
                    }
...
void fill_passport(Passport &passport,
    ...
    for (const Gtk::TreeModel::Row &station_row : klogic_row.children()) {
        if (passport.station_id != station_row[alarmobj_cols.id] ||
            !station_row.children())
            continue;
        if (passport_type == 111) {
        ...
        for (const Gtk::TreeModel::Row &controller_row :
            ...
            if (passport.group_id != controller_row[alarmobj_cols.id] ||
                !controller_row.children())
                continue;
            if (passport_type == 222) {
                for (const Gtk::TreeModel::Row &funcblock_row :
                     controller_row.children()) {
                    if (!funcblock_row.children())
                        continue;
                    for (const Gtk::TreeModel::Row &param_row :
                         funcblock_row.children()) {
            ...
            } else if (passport_type == 223) {
                for (const Gtk::TreeModel::Row &controller_child_row :
                     controller_row.children()) {
                    if (controller_child_row[alarmobj_cols.name] !=
                            "Служебные параметры" ||
...
bool parse_alarms_config(
    ...
    auto parse_group_recursive =
        ...
        if (alarms_elem) {
            ...
            for (tinyxml2::XMLElement *alarm_elem =
                ...
                AlarmRef alarmref = {
                    ...
                    alarm_string_id};
                used_params[convert_alarm_to_klogic_string_id(
                    alarm_string_id)] = alarmref;
            }
        }
        ...
        group_row[group_cols.block_by_passport_val] = block_by_passport_val;
        ...
        group_row[group_cols.checkback_val] = checkback_val;
        if (checkback_val != 0)
            passports[5] =
        ...
        group_row[group_cols.write_on_checkback] = write_on_checkback;
        if (write_on_checkback)
        ...
        for (tinyxml2::XMLElement *situation_elem =
            ...
            unsigned char alarm_confirm_interval =
            ...
            std::optional<int> intense_definition =
                GetChildOptInt(situation_elem, "IntenseDefinition");
            if (!intense_definition)
                new_situation_row[situation_cols.intense_absolute] = true;
            new_situation_row[situation_cols.intense_up] =
                GetChildDouble(situation_elem, "UpIntense");
            new_situation_row[situation_cols.intense_down] =
                GetChildDouble(situation_elem, "DownIntense");
            new_situation_row[situation_cols.alarm_kind] =
                GetChildInt(situation_elem, "AlarmKind");
            std::array<bool, 7> new_checkboxes = {
    ...
    auto write_group_recursive = [&](const auto &self,
        ...
        if (block_by_passport_val != 0) {
            ...
            if (block_passport.selected)
                append_passport_elem(default_alarm_child_elem, "BlockPassport",
                                     block_passport);
            append_val_elem(
                default_alarm_child_elem, "BlockByPasspValue",
                std::to_string(group_row[group_cols.block_by_passport_val])
                    .c_str());
        }
        ...
        if (checkback_val != 0) {
            ...
            if (checkback_passport.selected)
                append_passport_elem(default_alarm_child_elem,
                                     "CheckBackPassport", checkback_passport);
            append_val_elem(
                default_alarm_child_elem, "CheckBackValue",
                std::to_string(group_row[group_cols.checkback_val]).c_str());
        }
        bool write_on_checkback = group_row[group_cols.write_on_checkback];
        if (write_on_checkback) {
        ...
        if (liststore_situations->children().size() > 0) {
            ...
            for (const Gtk::TreeModel::Row &situation_row :
                ...
                if (checkboxes[2]) {
                    if (!situation_row[situation_cols.intense_absolute])
                        append_val_elem(situation_elem, "IntenseDefinition",
                                        "0");
                    if (situation_row[situation_cols.intense_up] != 0)
                        append_val_elem(
                            situation_elem, "UpIntense",
                            std::to_string(
                                situation_row[situation_cols.intense_up]));
                    if (situation_row[situation_cols.intense_down] != 0)
                        append_val_elem(
                            situation_elem, "DownIntense",
                            std::to_string(
                                situation_row[situation_cols.intense_down]));
                }
                if (checkboxes[0])
                ...
                if (checkboxes[4]) {
                    ...
                    append_val_elem(email_elem, "UseDefaultSubj",
                                    situation_params[1].bool1 ? "true"
                                                              : "false");
                    append_val_elem(email_elem, "UseDefaultText",
                                    situation_params[1].bool2 ? "true"
                                                              : "false");
...
bool parse_controller_config(
    ...
    const std::unordered_map<std::string, AlarmRef> &used_params,
    ...
    std::string &errors) {
    errno = 0;
    std::ifstream ifs(controller_config_path);
    int err = errno;
    if (!ifs.is_open() || err != 0) {
        errors += std::string("- Не удалось открыть файл конфигурации ") +
                  controller_config_path + ": " + strerror(err) + "\n";
        return false;
    }
    std::vector<char> buffer((std::istreambuf_iterator<char>(ifs)),
                             std::istreambuf_iterator<char>());
    ...
    if (result != tinyxml2::XML_SUCCESS) {
        errors += std::string("- Не удалось прочитать файл конфигурации ") +
                  controller_config_path + ": " + config.ErrorStr() + "\n";
        return false;
    }

    tinyxml2::XMLElement *root =
        config.FirstChildElement("KLogicGlobalArraySettingForMDD");
    if (!root)
        return false;
    tinyxml2::XMLElement *controller_elem =
    ...
    tinyxml2::XMLElement *controller_settings_elem =
        controller_elem->FirstChildElement("Settings");
    controller_row[alarmobj_cols.name] =
        controller_settings_elem
            ? GetChildText(controller_settings_elem, "Name")
            : Glib::ustring("Контроллер " + std::to_string(controller_idx));
    controller_row[alarmobj_cols.chipher] = controller_config_path;
    controller_row[alarmobj_cols.id] = controller_idx;

    tinyxml2::XMLElement *controller_globalarray_elem =
        controller_elem->FirstChildElement("GlobalArray");
    if (controller_globalarray_elem == nullptr)
        return false;
    tinyxml2::XMLElement *fabl_elem =
        controller_globalarray_elem->FirstChildElement("FABL");
    if (fabl_elem == nullptr)
        return false;
    for (tinyxml2::XMLElement *task_elem =
             fabl_elem->FirstChildElement("Object");
         task_elem != nullptr;
         task_elem = fabl_elem->NextSiblingElement("Object")) {
        Gtk::TreeModel::Row task_row =
            *treestore_zones->append(controller_row.children());
        task_row[alarmobj_cols.icon] = pixbuf_task;
        task_row[alarmobj_cols.name] = GetChildText(task_elem, "Name");
        ...
        auto append_service_param_row = [treestore_zones, treestore_stations,
                                         station_row, controller_row,
                                         service_params_row, pixbufs_params,
                                         pixbuf_discrete, pixbuf_analog,
                                         used_params, &group_cols,
                                         &alarmobj_cols](
            ...
            Glib::ustring service_param_string_id =
                station_row[group_cols.confname] + "." +
                controller_row[alarmobj_cols.name] + "." +
                "Служебные параметры." + name;
            auto alarmref_iter = used_params.find(service_param_string_id);
            ...
            param_row[alarmobj_cols.id] = alarmref_iter == used_params.end()
                                              ? 0
                                              : alarmref_iter->second.alarm_id;
            param_row[alarmobj_cols.userpath] =
                alarmref_iter == used_params.end()
                    ? ""
                    : alarmref_iter->second.userpath;
            param_row[alarmobj_cols.passport_id] = idx;
            ...
            param_row[alarmobj_cols.alarm_string_id] =
                "Kl\\St" + station_row[group_cols.station_id] +
                std::string("\\Cn") +
                std::to_string(static_cast<short unsigned int>(
                    controller_row[alarmobj_cols.id])) +
                "\\Служебные параметры\\\\" + name;
            param_row[alarmobj_cols.station_id] =
                static_cast<short unsigned int>(
                    station_row[group_cols.station_id]);
            param_row[alarmobj_cols.passport_type] = 222;
            param_row[alarmobj_cols.passport_group_id] =
                static_cast<short unsigned int>(
                    controller_row[alarmobj_cols.id]);

            if (alarmref_iter == used_params.end())
                return;
            Gtk::TreeModel::Path group_path =
                Gtk::TreeModel::Path(alarmref_iter->second.userpath);
            ...
            alarm_row[alarmobj_cols.id] = alarmref_iter->second.alarm_id;
            ...
            alarm_row[alarmobj_cols.alarm_string_id] =
                alarmref_iter->second.alarm_string_id;
            alarm_row[alarmobj_cols.klogic_string_id] = service_param_string_id;
            alarm_row[alarmobj_cols.passport_id] =
                alarmref_iter->second.passport_id;
            alarm_row[alarmobj_cols.station_id] =
                static_cast<short unsigned int>(
                    station_row[group_cols.station_id]);
            alarm_row[alarmobj_cols.passport_type] = 222;
            alarm_row[alarmobj_cols.passport_group_id] =
                static_cast<short unsigned int>(
                    controller_row[alarmobj_cols.id]);
            group_row[group_cols.alarms] = liststore_alarms;
        };
        append_service_param_row(1, "Канал связи",
                                 "Тип канала связи: 0 - TCP/IP, 1 - COM-порт",
                                 2, 0);
        append_service_param_row(2, "Инициативный опрос",
                                 "Команда на инициативный опрос контроллера", 1,
                                 0);
        append_service_param_row(3, "Длительность цикла опроса",
                                 "Длительность последнего цикла опроса, мсек",
                                 2, 1);
        append_service_param_row(4, "Связь с контроллером",
                                 "Наличие связи с контроллером", 1, 1);
        append_service_param_row(5, "Приостанов опроса", "Приостанов опроса", 1,
                                 0);
        append_service_param_row(6, "Режим генерации демо-значений",
                                 "Режим генерации демонстрационный значений", 1,
                                 0);
        append_service_param_row(7, "Период опроса",
                                 "Период полного принудительного опроса, сек",
                                 2, 0);
        ...
        append_service_param_row(21, "Опрос",
                                 "Опрос (для GSM - регулярный опрос "
                                 "без разрыва соединения",
                                 1, 0);
        append_service_param_row(
            22, "Активный контроллер",
            "Активный контроллер: 0 - нет, 1 - основной, 2 "
            "- дублер/резерв. 3 - оба",
            2, 1);
        append_service_param_row(25, "Связь с сервером",
                                 "Наличие связи с сервером", 1, 1);

        std::string station_confname =
            static_cast<Glib::ustring>(station_row[group_cols.confname]);
        for (tinyxml2::XMLElement *signal_elem =
                 task_elem->FirstChildElement("Object");
             signal_elem != nullptr;
             signal_elem = signal_elem->NextSiblingElement("Object")) {
            Gtk::TreeModel::Row fb_row =
                *treestore_zones->append(task_row.children());
            fb_row[alarmobj_cols.icon] = pixbuf_fb;
            fb_row[alarmobj_cols.name] = GetChildText(signal_elem, "Name");
            for (tinyxml2::XMLElement *param_elem =
                     signal_elem->FirstChildElement("Var");
                 param_elem != nullptr;
                 param_elem = param_elem->NextSiblingElement("Var")) {
                tinyxml2::XMLElement *param_settings_elem =
                    param_elem->FirstChildElement("Settings");
                if (!param_settings_elem)
                    continue;
                std::string string_id =
                    station_confname + "." +
                    GetChildText(param_settings_elem, "Name").data();
                auto alarmref_iter = used_params.find(string_id);
                if (alarmref_iter == used_params.end())
                    continue;
                Gtk::TreeModel::Path group_path =
                    Gtk::TreeModel::Path(alarmref_iter->second.userpath);
                ...
                // Добавление параметров в дерево KLogic
                Gtk::TreeModel::Row param_row =
                    *treestore_zones->append(fb_row.children());
                param_row[alarmobj_cols.alarmobj_type] =
                    ALARMOBJTYPE_KLOGICPARAM;
                param_row[alarmobj_cols.userpath] =
                    treestore_stations->get_path(group_row).to_string();
                unsigned int type = GetChildInt(param_settings_elem, "Type");
                param_row[alarmobj_cols.type] = type;
                bool out = GetChildBool(param_settings_elem, "Out");
                param_row[alarmobj_cols.out] = out;
                int param_idx = type * 2 + out;
                if (param_idx > 5)
                    param_idx = 0;
                param_row[alarmobj_cols.icon] = pixbufs_params[param_idx];
                param_row[alarmobj_cols.id] = alarmref_iter->second.alarm_id;
                param_row[alarmobj_cols.name] =
                    GetChildText(param_settings_elem, "ShortName");
                param_row[alarmobj_cols.chipher] =
                    GetChildText(param_settings_elem, "ShortName");
                param_row[alarmobj_cols.fullname] =
                    GetChildText(param_settings_elem, "Description");
                param_row[alarmobj_cols.station_name] =
                    static_cast<Glib::ustring>(station_row[group_cols.name]);
                param_row[alarmobj_cols.passport_val_type_str] =
                    get_passport_val_type_str(type, out);
                ...
                // Добавление алармов в список паспортов
                if (!group_row)
                    continue;
                Glib::RefPtr<Gtk::ListStore> liststore_alarms =
                    group_row[group_cols.alarms];
                ...
                alarm_row[alarmobj_cols.icon] =
                    type == 1 ? pixbuf_discrete : pixbuf_analog;
                alarm_row[alarmobj_cols.name] =
                    GetChildText(param_settings_elem, "ShortName");
                alarm_row[alarmobj_cols.chipher] =
                    GetChildText(param_settings_elem, "ShortName");
                alarm_row[alarmobj_cols.fullname] =
                    GetChildText(param_settings_elem, "Description");
                alarm_row[alarmobj_cols.station_name] =
                    static_cast<Glib::ustring>(station_row[group_cols.name]);
                alarm_row[alarmobj_cols.passport_val_type_str] =
                ...
                alarm_row[alarmobj_cols.alarm_string_id] =
                    alarmref_iter->second.alarm_string_id;
                alarm_row[alarmobj_cols.klogic_string_id] = string_id;
                alarm_row[alarmobj_cols.passport_id] =
                    alarmref_iter->second.passport_id;
                alarm_row[alarmobj_cols.station_id] =
                    static_cast<short unsigned int>(
                        station_row[group_cols.station_id]);
                alarm_row[alarmobj_cols.passport_type] = 223;
                alarm_row[alarmobj_cols.passport_group_id] =
                    static_cast<short unsigned int>(
                        controller_row[alarmobj_cols.id]);
                group_row[group_cols.alarms] = liststore_alarms;
...
bool parse_klogic_configs(
    ...
    for (const Gtk::TreeModel::Row &station_row :
        ...
        new_station_row[alarmobj_cols.id] =
        ...
        for (int i = 0; i < n; i++) {
            ...
            (void)parse_controller_config(
                ...
                pixbuf_analog, pixbufs_params, alarmobj_cols, group_cols,
                errors);
        }
    }
    return errors.empty();
